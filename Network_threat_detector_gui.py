...
class IDSApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.upload_btn = tk.Button(panel, text="Upload CSV", command=self.upload_csv,
                                    bg="#8b5cf6", fg="#000", font=("Segoe UI", 11, "bold"))
        ...
        self.upload_btn.grid(row=0, column=3, padx=8, pady=8, sticky="w")
        ...
        self.stat_total.grid(row=0, column=4, padx=8, sticky="e")
        self.stat_anom.grid(row=0, column=5, padx=8, sticky="e")
        self.stat_rate.grid(row=0, column=6, padx=8, sticky="e")
    ...
    def upload_csv(self):
        ...
        self.stop_monitor()
        self.clear_table()
        self.status.config(text=f"Sequentially processing {os.path.basename(file_path)}...")
        self.update_idletasks()
        
        self.running = True
        threading.Thread(target=self._process_file_sequential, args=(file_path,), daemon=True).start()

    def _process_file_sequential(self, file_path):
        try:
            if file_path.lower().endswith('.xlsx'):
                try:
                    ...
                    self.running = False
                    self.after(0, lambda: messagebox.showerror("Error", "openpyxl is required to read Excel files. Install with 'pip install openpyxl'"))
                    return
            else:
            ...
            df["protocol_enc"] = df.get("protocol", "tcp").astype(str).str.lower().map(PROTO_MAP).fillna(0).astype(int)
            for col in NUM_FEATURES:
                df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0.0)
            
            X = df[["protocol_enc"] + NUM_FEATURES].values
            ...
            for i in range(total_rows):
                ...
                row_dict["protocol"] = row_dict.get("protocol", "tcp")
                score = float(scores[i])
                pred = int(preds[i])
                label = "ANOMALY" if pred == -1 else "OK"
                
                # Push to queue to be displayed sequentially by the GUI thread
                self.q.put((row_dict, score, label))
                time.sleep(0.05) # Delay to simulate sequential reading
                
            self.running = False
            self.after(0, lambda: self.status.config(text=f"Finished processing {os.path.basename(file_path)}."))
            
        except Exception as e:
            self.running = False
            self.after(0, lambda err=e: messagebox.showerror("Error", f"Failed to process file:\n{err}"))
